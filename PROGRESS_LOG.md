...
## REAL LOGO SWAP + BOOK-COVER CROP FIX + FULL BILINGUAL SWEEP — 18 Jul 2026 (same day, follow-up)

Suraj sent 4 more points in one message; all 4 addressed in this pass.

**1. Real logo swap (correcting the previous placeholder).** Suraj rejected the earlier CSS-recreated "अ" mark as "ye logo nahi hai" and pointed to the actual official file on his own computer: `D:\OneDrive\00 Maa\03. Maa Sampurn Sangrah\logo.png`. Staged that file read-only via the device bridge, confirmed it (1120×1174 RGBA) as the real brand emblem. Circular-cropped it with a 4x-supersampled alpha mask for clean anti-aliased edges, compressed with `pngquant`, and produced: `assets/img/logo-mark.png` (300×300, ~57KB — used for the header `.brand-mark` and the floating-button `.float-icon` on all 38 pages) plus regenerated `favicon-32.png`, `favicon-180.png`, `favicon-512.png`, and a fresh multi-size `favicon.ico` (16/32/48/64), all from the real logo. Removed the now-orphaned CSS-drawn SVG favicon `<link>` from all 38 pages (reverted to PNG-only favicon links). **Disclosure for Suraj:** at 32px (favicon) and 34px (floating button icon) the ornate circular emblem reads as a soft round gold-toned badge rather than crisp legible detail — this is a genuine physical constraint of fine detail at small pixel sizes, not a bug. The logo is fully sharp and detailed everywhere it's shown larger (header, 300px master file). Flagging this rather than deciding quietly — happy to make a simplified high-contrast mark for the tiny sizes only, if you want.

**2. Book cover crop fix ("cover page pura nahi aa raha").** Root cause: `.book-cover` had a fixed `height: 210px` + `object-fit: cover`, which aggressively cropped the portrait-oriented cover images (real aspect ratios ~0.71–0.73, e.g. Sitayan 608×858, Radhavtaram 607×860, Mahasati Gauri 669×920) on `index.html`'s featured cards and `granth-sangrah.html`'s listing grid. Fixed by switching to `aspect-ratio: 0.71/1` with `height: auto` and `object-fit: contain` — covers now display in full, uncropped, letterboxed on a soft maroon gradient background if needed. Book detail pages already had inline overrides and were unaffected. Verified via local Playwright screenshot showing complete cover art.

**3. Comprehensive bilingual sweep — the real fix for "english select hone k baad bhi hindi aa raha hai... isko puri website par thik nahi kar paya hai tu".** Ran a full sitewide `data-i18n-hi` inventory and confirmed Suraj was right: the earlier "book descriptions made bilingual" pass only covered 9 pages — 28 other pages (all utility/policy pages + all 18 blog articles) were still 100% Hindi-only in body content, with only nav/header/footer toggling. Fanned out 28 parallel subagents (one per page, same proven pattern) and translated every page in full: `privacy-policy.html`, `terms.html`, `refund-policy.html`, `contact.html`, `gallery.html`, `search.html` (including the JS-rendered live search-result templates), `404.html`, `navratri-path-suchi.html`, `shravan-path-suchi.html`, `blog.html` (listing page, all 18 card excerpts), and all 18 individual blog articles (51-shakti-peeth-katha, bansuri-ki-pukar, daksh-yagya-atmadah, durga-saptashati-vidhi, govardhan-leela-sharanagat, grihasth-vairagya-samarpan, kaikeyi-pashchatap, kritagyata-sadhna-ki-neev, nau-durga-swaroop-jeevan-sutra, navratri-mahatva, parvati-shiv-vivah-divya-leela, parvati-tapasya-shiv-prapti, raas-leela-prem-path, radha-janam-swarn-kamal, sati-janam-adishakti, sita-janam-rahasya, sita-sutradhar, swayamvar-dhanush-bhang). Verified sitewide: all 38 HTML files have exactly matching `data-i18n-hi`/`data-i18n-en` counts (no orphaned spans) and all 38 parse as valid HTML (Python `html.parser` tag-balance sweep, zero errors). One subagent flagged a suspicious injected instruction that appeared mid-task claiming a different file needed an unrelated change — correctly ignored as not a genuine task instruction; noted here for visibility.

**4. Razorpay combo payment links — still open, not actioned.** Creating a live Razorpay Payment Link requires Suraj's own login/session (financial/payment-account action) — this needs him present and logged in; not something to complete unattended. Will action as soon as he's ready.

**Deployed live** via the existing `sap-site-deploy` token — combined push covering all of the above (logo assets, book-cover CSS fix, 28-page bilingual sweep).

**Still open:** Razorpay combo links (needs Suraj), `sap-site-deploy` token keep/delete decision (asked once before, unanswered), the 32px-favicon legibility tradeoff (flagged above, awaiting Suraj's call on whether to simplify), build-order items 3–8 from the earlier list (testimonial form + आज का श्लोक widget, About page personal-journey draft approval, PWA, PageSpeed check, audio clips, Google Business Profile draft).
